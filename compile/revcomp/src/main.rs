...
mod libc {
    ...
    unsafe extern "C" { pub fn memchr(cx: *const c_void, c: c_int, n: size_t) -> *mut c_void; }
}
...
fn reverse_complement(seq: &mut [u8], tables: &Tables) {
    ...
    let mut right = len;
    while left < right {
        let left_byte = seq[left];
        let right_byte = seq[right - 1];
        seq[left] = tables.cpl8(right_byte);
        seq[right - 1] = tables.cpl8(left_byte);
        left += 1;
        right -= 1;
    }
}

/// Executes a closure over the given iterator of mutable slices.
